...
_DEPS=str.h hash.h ujson-encode.h ujson-extract.h ujson-data.h ujson-parse.h hexdump.h endian.h udp.h schematags.h ujsizes.h movebytes.h
IDIR=inc
DEPS=$(pasubst %,$(IDIR)/%,$(_DEPS))

# List of source files, excluding the EXE and TESTS runnables above
_SOURCES=str.c hash.c ujson-encode.c ujson-extract.c ujson-data.c ujson-parse.c hexdump.c udp.c endian.c movebytes.c
SDIR=src
